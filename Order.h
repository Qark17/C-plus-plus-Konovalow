#pragma once // Регистрация ЗАКАЗОВ
#include <string>
#include <vector>
#include <ctime>
#include <memory>
...
struct OrderItem {
    ...
    int quantity;
    double unitPrice;
    OrderItem(const std::shared_ptr<ProductBase>& prod, int qty, double price)
        : product(prod), quantity(qty), unitPrice(price) {    }
    double getItemTotal() const {return quantity * unitPrice;    }
};
class Order {
private:
...
public:
    ...
    Order(const std::string& shop, const std::shared_ptr<ProductBase>& product,
    ...
    void addItem(const std::shared_ptr<ProductBase>& product, int quantity, double unitPrice);
    ...
    OrderItem* findItem(const std::shared_ptr<ProductBase>& product);
    OrderItem* findItem(const std::string& productCode);
    bool containsItem(const std::shared_ptr<ProductBase>& product) const;
