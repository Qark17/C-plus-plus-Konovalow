...
Order::Order(const std::string& shop, const std::string& productCode,
    ...
    if (quantity > 0 && unitPrice >= 0) {
        items.emplace_back(productCode, quantity, unitPrice);
        recalculateTotal();
    }
}
...
void Order::addItem(const std::string& productCode, int quantity, double unitPrice) {
...
void Order::addItem(const OrderItem& item) {
    if (item.quantity <= 0 || item.unitPrice < 0) return;
...
std::string Order::getDetailedInfo() const {
    ...
    if (!items.empty()) {
        ...
        int counter = 1;
        for (const auto& item : items) {
            info += "\n" + std::to_string(counter++) + ". " + item.productCode +
